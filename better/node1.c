...
printdt1(&table1); 
  ...
  for (int i = 0; i < 4; i++) {
    int new_cost = table1.costs[routing1[j]][j]+rcvdpkt->mincost[i];

    if (table1.costs[routing1[i]][j] > new_cost) {
      routing1[i] = j;
      changed = 1;
    }
    else if(routing1[i] == j && new_cost >table1.costs[routing1[i]][i]){
      int min = j;
      for(int k=0; k<4; k++){
        if(k!= 0 && table1.costs[k][i]< table1.costs[min][i]){
          min = k;
        }
      }
      changed =1;
